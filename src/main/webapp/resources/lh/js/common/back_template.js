...
function doSearch(){
	...
	if(beforeSearch){//拦截：执行查询之前 执行此方法
		var flag = beforeSearch(common_queryObj);
		if(!flag)return;
	}
...
function clearSearch(){
	if(beforeClearSearch){//拦截：执行清除查询之前 执行此方法
		var flag = beforeClearSearch(common_queryObj);
		if(!flag)return;
	}
...
/** 设置主对象表单的字段值 */
function setMainObj(data, operation, isReadOnly){
	lh.setField({data : data, inputsDom : lh.$mainObj});
	if(afterSetMainObj){//拦截：表单设置完毕执行方法
		afterSetMainObj(data, operation, isReadOnly);
	}
}
...
function openMainObjWin(index, operation){
	...
	$form.form({
	    ...
	    onSubmit: function(){
	       ...
	       if(flag){
	       		...
	       		if(beforeAddOrUpdate){//拦截：新增修改操作执行之前的拦截方法，返回false则不执行新增修改
	       			var flag = beforeAddOrUpdate(mainObj);
	       			if(!flag)return;
	       		}
	       		
	       		lh.post('back', lh.config.addOrUpdateUrl, mainObj, function(rsp){
	       			if(afterAddOrUpdate){//拦截：新增修改操作执行完成之后执行的回调方法
		       			var flag = afterAddOrUpdate(mainObj, rsp);
	...
	if(operation == 'add'){//添加
		...
		if(afterOpenAddWin){//拦截：打开添加窗口执行此方法
   			afterOpenAddWin(data, operation, isReadOnly);
   		}
	}else{//查看或修改
		...
		setMainObj(data, operation, isReadOnly);//设置表单字段值
	}
