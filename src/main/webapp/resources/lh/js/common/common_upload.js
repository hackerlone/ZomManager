...
function initOption(option, random){
	...
	if(option.chooseBtnDomId)opt.chooseBtnDomId = option.chooseBtnDomId;
...
function initUploadSimple(option) {
	initUpload('/manager/common/upload', option);//调用完整方法
}
...
function initUploadAllFile(option) {
	initUploadFile('/manager/common/upload?folderType=2', option);//调用完整方法
}
...
function initUpload(url, option) {
	...
	if(!url)url = '/manager/common/upload';
	...
	var uploader = new plupload.Uploader({
				...
				max_retries : 2,
				filters : {
					mime_types:[{title:'fileType', extensions:'xlsx,xls'}],
