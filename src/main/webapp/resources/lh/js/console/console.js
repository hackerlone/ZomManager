/** 基础数据和基础设置 */
lh.config = {
	mainObjLowerName : 'console',
	mainObjUpperName : 'Console'
}
lh.gridParam = {};
$(function() {
	...
	var url = "";
	if(isRank1 == 'true'){
		 url = "/admin/console/getConsoleList"
	}else{
		 url = "/admin/console/getConsoleList2"
	}
	loadMainGrid(url);
	
});

function loadMainGrid(url){
	var url = lh.getCmsUrl(url);
	$('#datagrid').dataTable( {
		serverSide: true,
		processing: false,
		paging:true,
            ...
            { "data": "userPriority" },
            {
	            ...
	            render: function(data, type, row, meta) {
	                 var dom = '<div style="text-align:center;">' +
	            				'<a class="btn btn-xs btn-success" onclick="lh.cmsJump(\'/admin/console/update?id='+row.id+'\');" href="javascript:" title="编辑"><i class="ace-icon fa fa-pencil bigger-120"></i></a>'+
...
function doSearch(){
	...
	lh.gridParam = {nicknameLike:nicknameLike,status:status,username:username};
	reloadMainGrid();
}
...
function deleteById(id){
	...
	lh.post('back', lh.getCmsUrl('/admin/console/doDelete'), {id:id}, function(rsp){
		 if(rsp.success){
			reloadMainGrid();
		 }else{
			 lh.alert(rsp.msg);
...
function updateMainStatus(status, id){
	var url  = lh.getCmsUrl('/admin/console/updateToggleStatus') + '/' + id;
