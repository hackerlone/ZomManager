/** 基础数据和基础设置 */
lh.config = {
	mainObjLowerName : 'group',
	mainObjUpperName : 'Group'
}
lh.gridParam = {};
$(function() {
...
function loadMainGrid(){
	var url = lh.getCmsUrl("/admin/group/getGroupList");
	$('#datagrid').dataTable( {
		serverSide: true,
		processing: false,
		paging:true,
       ...
       columns: [
            ...
            { "data": "createDate" , render: function(data, type, row, meta) {
	            return lh.formatDate(data);
	        }},
            {
	            "data": null,
	            render: function(data, type, row, meta) {
	                 var dom = '<div style="text-align:center;">' +
	            				'<a class="btn btn-xs btn-success" onclick="lh.cmsJump(\'/admin/group/update?id='+row.id+'\');" href="javascript:" title="编辑"><i class="ace-icon fa fa-pencil bigger-120"></i></a>'+
...
function doSearch(){
	...
	lh.gridParam = {groupName:nicknameLike,status:status};
	reloadMainGrid();
}
...
function deleteById(id){
	...
	lh.post('back', lh.getCmsUrl('/admin/group/doDelete'), {id:id}, function(rsp){
		 if(rsp.success){
			reloadMainGrid();
		 }else{
			 lh.alert(rsp.msg);
...
function updateMainStatus(status, id){
	var url  = lh.getCmsUrl('/admin/group/updateToggleStatus') + '/' + id;
