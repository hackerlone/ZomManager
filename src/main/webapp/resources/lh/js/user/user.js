...
function loadMainGrid(){
	var url = lh.getCmsUrl("/admin/user/getUserList");
	$('#datagrid').dataTable( {
		serverSide: true,
		processing: false,
		paging:true,
       ...
       columns: [
            ...
            { "data": "nickname" },
	         { "data": "status", render: function(data, type, row, meta) {
            	if(data == 0){
            		 return '<span style="color:orange;">停用</span><span style="color: #307ED3;margin-left: 15px;cursor: pointer;" onclick="updateMainStatus(1,'+row.id+');">启用</span>';
            	}else if(data == 1){
            		 return '<span style="color:green;">启用</span><span style="color: #307ED3;margin-left: 15px;cursor: pointer;" onclick="updateMainStatus(0,'+row.id+');">停用</span>';
            	}
	        }},
            { "data": "phone" },
            { "data": "userPriority" },
            ...
            { "data": "groupNames" },
            {
	            ...
	            render: function(data, type, row, meta) {
	                var dom = '<div style="text-align:center;">' +
	            				'<a class="btn btn-xs btn-success" onclick="lh.cmsJump(\'/admin/user/update?id='+row.id+'\');" href="javascript:" title="编辑"><i class="ace-icon fa fa-pencil bigger-120"></i></a>'+
	            				'<a style="margin-left:5px;" class="btn btn-xs btn-primary" onclick="lh.cmsJump(\'/admin/user/updatePwd?id='+row.id+'\');" href="javascript:"title="修改密码"><i class="ace-icon fa fa-lock bigger-120"></i></a>'+
		            		    '<a style="margin-left:5px;" class="btn btn-xs btn-danger" href="javascript:" onclick="confirmDelete('+row.id+');" title="删除"><i class="ace-icon fa fa-trash-o bigger-120"></i></a>' +
		            		  '</div>';
	                return dom;
	            }
            }
...
function doSearch(){
	...
	lh.gridParam = {nicknameLike:nicknameLike,status:status,phoneLike:phoneLike};
	reloadMainGrid();
}
...
function updateMainStatus(status, id){
	var url  = lh.getCmsUrl('/admin/user/updateToggleStatus') + '/' + id;
	lh.post('back', url, {id:id}, function(rsp){
		 if(rsp.success){
			reloadMainGrid();
		 }else{
			 lh.alert(rsp.msg);
...
function deleteById(id){
	...
	lh.post('back', lh.getCmsUrl('/admin/user/doDelete'), {id:id}, function(rsp){
		 if(rsp.success){
			reloadMainGrid();
		 }else{
			 lh.alert(rsp.msg);
