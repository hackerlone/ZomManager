...
public class PluploadUtil {
     ...
     * 保存上传文件，兼合并功能
     */
    private static void saveUploadFile(InputStream input, File saveFile, boolean append) throws IOException {
        OutputStream out = null;
        try {
            if (saveFile.exists() && append) {
                out = new BufferedOutputStream(new FileOutputStream(saveFile, true), BUF_SIZE);
            } else {
                out = new BufferedOutputStream(new FileOutputStream(saveFile), BUF_SIZE);
            }
            byte[] buffer = new byte[BUF_SIZE];
            int len = 0;
            while ((len = input.read(buffer)) > 0) {//写入文件
